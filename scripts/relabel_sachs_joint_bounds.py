"""Reuse accepted observational Sachs datasets for a joint-intervention query.

This script reconstructs accepted CTM structural models from an existing
generated-data root, computes new Sachs joint-intervention bound diagnostics,
...
def _target_trial_dir(output_root, graph, n, dim, trial_index, bound_do, v_sizes=None, dimension_label=None):
    label = _bound_do_label(bound_do)
    if dimension_label is None:
        dimension_label = _dimension_label(dim, v_sizes)
    return Path(output_root) / "graph={}-n_samples={}-dim={}-bound_do={}-trial_index={}".format(
        graph, n, dimension_label, label, int(trial_index))
...
def _process_trial(source_dir, args):
    ...
    if metadata.get("graph") != "sachs":
        raise ValueError("expected sachs metadata in {}, got {}".format(
    ...
    graph_file = metadata.get("graph_file", "dat/cg/sachs.cg")
    ...
    target_dir = _target_trial_dir(
        ...
        int(metadata["dim"]),
        int(metadata["trial_index"]),
        args.bound_do,
        explicit_v_sizes,
        dimension_label=metadata.get("dimension_label"))
    target_dir.mkdir(parents=True, exist_ok=True)

    artifact_actions = _reuse_large_artifacts(
        source_dir, target_dir, "sachs", overwrite=args.overwrite, copy=args.copy)
    _copy_positivity_diagnostics(source_dir, target_dir, "sachs")
    ...
    _write_json(target_dir / "sachs_data_metadata.json", updated_metadata)
    _write_json(target_dir / "ground_truth_bounds.json", bound)
    _write_json(target_dir / "sachs_ground_truth_bounds.json", bound)
    _write_json(target_dir / "adjustment_separation_diagnostics.json", adjustment)
    _write_json(target_dir / "sachs_adjustment_separation_diagnostics.json", adjustment)
    return target_dir, bound, adjustment


def build_parser():
    parser = argparse.ArgumentParser(
        description="Reuse accepted observational Sachs CTM datasets for joint Sachs bounds.")
    parser.add_argument(
        "--source-root",
        default="out/paper_bound_datasets_adjustment_gap",
        help="root containing accepted Sachs dataset directories")
    parser.add_argument(
        "--output-root",
        default="out/paper_bound_datasets_sachs_joint_obs",
...
def main():
    ...
    args.bound_do = {"PKA": 0, "PKC": 0}
    if args.bound_mc_samples <= 0:
        raise ValueError("--bound-mc-samples must be positive")

    source_dirs = _source_trial_dirs(args.source_root, "sachs")
    if args.trial_index:
    ...
    if not source_dirs:
        raise FileNotFoundError("no Sachs trial directories found in {}".format(args.source_root))
