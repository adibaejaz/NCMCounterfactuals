...
EXP_NAME="${EXP_NAME:-overnight_runs/coupling/paper_bound_masked_ff_barley_joint_e50_coupled_v1_querymask_mfit1_1x1_uniform}"
DATA_ROOT="${DATA_ROOT:-out/paper_bound_datasets_barley_joint_obs}"
SESSION="${SESSION:-barley_coupled_v1_gpu1_trial0_rerun1_e50}"
...
command_for_barley() {
  printf "%s" "${PYTHON_BIN} src/masked_experiment.py ${EXP_NAME} \
    ...
    --mask-init-low 0.1 \
    --mask-init-high 0.9 \
    --mask-equiv-class-file dat/cg/barley_equiv.cg \
    --mask-l1-lambda 0 \
    --alt-opt \
    ...
    --early-stop-patience ${EARLY_STOP_PATIENCE}$(trial_args)"
}

plan() {
  echo "Barley coupled-v1 masked FF-NCM plan"
  ...
  echo "THETA_ONLY_EXTRA_EPOCHS=${THETA_ONLY_EXTRA_EPOCHS}"
...
launch() {
  ...
  if [[ ! -f dat/cg/barley.cg ]]; then
    echo "[missing] ${ROOT_DIR}/dat/cg/barley.cg" >&2
    exit 1
  fi
  if [[ ! -f dat/cg/barley_equiv.cg ]]; then
    echo "[missing] ${ROOT_DIR}/dat/cg/barley_equiv.cg" >&2
    exit 1
  fi
  if [[ ! -d "$DATA_ROOT" ]]; then
