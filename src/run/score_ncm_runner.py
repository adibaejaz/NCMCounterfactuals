...
class ScoreNCMRunner(BaseRunner):
    ...
    def generate_data(self, true_graph, num_samples, dim, trial_index, hyperparams=None, \
        ...
        dir = 'out/%s' % key # name of the output directory
        data_filepath = '%s/data.th' % dir  # name of the data file
        model_filepath = '%s/dat_m.th' % dir

        with self.lock(f'{dir}/lock', lockinfo) as acquired_lock:
    ...
    def test_graph(self, dat_m, dat_sets, true_graph, test_graph, num_samples, dim, trial_index, \
                  gpu=None, hyperparams=None, lockinfo=os.environ.get('SLURM_JOB_ID', ''), verbose=False):
        
        key = self.get_key(true_graph, num_samples, dim, trial_index)
        model_dir = 'out/%s/%s/' % (key, test_graph)

        with self.lock(f'{model_dir}/lock', lockinfo) as acquired_lock:
            if not acquired_lock:
                print('[locked]', model_dir)
                return
            
            print(f'Training model for {key} and {test_graph}')
            ...
            try:
                ...
                print(f'Model trained and saved for {key} and {test_graph}')

            except Exception as e:
                print(f'Error running score: {e} for {key} and {test_graph}')
            

    def run_score(self, true_graphs, test_graphs, num_samples, dim, num_trials, \
                  gpu=None, hyperparams=None, lockinfo=os.environ.get('SLURM_JOB_ID', ''), verbose=False):
        ...
        for true_graph in true_graphs:
            for trial_index in range(num_trials):
                ...
                for test_graph in test_graphs:
                    self.test_graph(dat_m, dat_sets, true_graph, test_graph, num_samples, dim, trial_index, \
